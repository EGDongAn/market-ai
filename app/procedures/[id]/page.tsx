...
export default function EditProcedurePage() {
  ...
  const fetchProcedure = useCallback(async () => {
    try {
      ...
      // Set form data
      setSelectedCategory(data.subcategory.category.id.toString())
      setFormData({
        subcategory_id: data.subcategory.id.toString(),
        name: data.name,
        aliases: data.aliases && Array.isArray(data.aliases)
          ? data.aliases.join(', ')
          : '',
        brand: data.brand || '',
        unit: data.unit || ''
  ...
  useEffect(() => {
    fetchCategories()
    fetchProcedure()
  }, [fetchCategories, fetchProcedure])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || '시술 수정에 실패했습니다.')
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-green-800">시술 수정</h1>
      </div>
